...
-- Función para listar los vehículos actualmente en el parqueadero
listarVehiculosEnParqueadero :: [Vehiculo] -> IO ()
listarVehiculosEnParqueadero parqueadero = do
    let vehiculosEnParqueadero = filter (isNothing . salida) parqueadero
    putStrLn "Vehículos actualmente en el parqueadero:"
    mapM_ (putStrLn . mostrarVehiculo) vehiculosEnParqueadero
    where
        isNothing Nothing = True
        isNothing _       = False
...
cicloPrincipal parqueadero = do
    ...
    case opcion of
        ...
        "4" -> do
            listarVehiculosEnParqueadero parqueadero
            cicloPrincipal parqueadero
